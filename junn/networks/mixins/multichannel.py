...
class MultichannelHandling(TilebasedNetwork):
    def get_training_fn(self, validation: bool = False):
        ...
        if output_channels > 1:
            ...
            def _inner(image, labels):
                ...
                labels = tf.cast(tf.one_hot(tf.cast(labels[:, :, 0], tf.int32), depth=output_channels), tf.float32)
                return image, labels
        else:
            @tf.function
            def _inner(image, labels):
    ...
    def get_prediction_fn(self):
        ...
        output_channels = self.parameters['output_channels'] if 'output_channels' in self.parameters else 1
        rebinarize_output = self.parameters['rebinarize_output'] if 'rebinarize_output' in self.parameters else True
        ...
        if output_channels > 1:
            ...
            def _inner(image):
                ...
                if rebinarize_output:
                    prediction = tf.where(tf.equal(prediction, REGION_FOREGROUND), 1.0, 0.0)
