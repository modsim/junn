...
from ...common.functions import convolve, get_gaussian_kernels, pad_to
from ...common.functions.affine import (
    tfm_identity,
    tfm_reflect,
    tfm_rotate,
    tfm_scale,
...
class TilebasedTrainingMixin:
    def get_training_fn(self, validation: bool = False):
        ...
        gaussian_kernels = get_gaussian_kernels(
            BlurMinimumSigma.value, BlurMaximumSigma.value, BlurSteps.value
        )
        ...
        def create_training_data(image, labels):
            ...
            def random_float(minval=0.0, maxval=1.0, seed=None):
                return tf.random.uniform(
                    shape=[], minval=minval, maxval=maxval, dtype=tf.float32, seed=seed
                )
            ...
            if random_float(seed=40) < RotateProbability.value:
                mat @= tfm_rotate(
                    random_float(
                        minval=RotateMinimum.value, maxval=RotateMaximum.value, seed=41
                    ),
                    shape=current_shape,
                )
            ...
            if random_float(seed=50) < ShearProbability.value:
                mat @= tfm_shear(
                    random_float(
                        minval=ShearXAngleMinimum.value,
                        maxval=ShearXAngleMaximum.value,
                        seed=51,
                    ),
                    random_float(
                        minval=ShearYAngleMinimum.value,
                        maxval=ShearYAngleMaximum.value,
                        seed=52,
                    ),
                    shape=current_shape,
                )
            ...
            if random_float(seed=60) < ScaleProbability.value:
                mat @= tfm_scale(
                    random_float(
                        minval=ScaleMinimum.value, maxval=ScaleMaximum.value, seed=61
                    ),
                    shape=current_shape,
                )
            ...
            image = image[
                margins[0] // 2 : -margins[0] // 2,
                margins[1] // 2 : -margins[1] // 2,
                :,
            ]
