...
def unet_unit(
    ...
    second_batch_norm = BatchNormalization(
        name='%s_batch_normalization_second' % (name,)
    )
    ...
    if recurrent_num:
        tensor = RecurrentConvolution2D(recurrent_num=recurrent_num, **parameters)(
            tensor
        )
    else:
...
def unet_level(
    ...
    tensor = configured_unet_unit(
        tensor,
        dropout=dropout_down,
        name='%s_level_%d_down'
        % (
            name,
            total_levels - level,
        ),
    ...
    if level > 0:
        if not just_convolutions:
            ...
            tensor = Conv2D(
                filters, 3, strides=2, padding='same', activation=activation
            )(tensor)

        tensor = unet_level(
            tensor,
            filters=int(filters * level_factor),
            level=level - 1,
...
def unet(
    ...
    **kwargs
    ...
    if not categorical:
        ...
        tensor = Conv2D(filters=output_channels, kernel_size=1, activation='linear')(
            tensor
        )
